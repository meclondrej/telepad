...
public class TelepadConnectCommand extends AbstractCommandHandler {
    ...
    public boolean handle(CommandSender exec, Command cmd, String alias, String[] args) {
        ...
        for (Telepad telepad : TelepadManager.telepads) {
            if (telepad.label().equals(args[1])) firstTelepad = telepad;
            if (telepad.label().equals(args[2])) secondTelepad = telepad;
            if (firstTelepad != null && secondTelepad != null) break;
        }
    ...
    public List<String> onTabComplete(
        ...
        if (args.length == 3)
            return TelepadManager.telepads.stream()
                    .filter(x -> !x.label().equals(args[1]))
                    .map(x -> x.label())
                    .collect(Collectors.toList());
