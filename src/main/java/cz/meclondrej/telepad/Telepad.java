...
public record Telepad(Location location, String label) {
    ...
    public void teleportEntities(Telepad target, Collection<Entity> entities) {
        for (Entity entity : entities) {
            Location location =
                    new Location(
                            target.location().getWorld(),
                            (double) (target.location().getBlockX())
                                    + (entity.getLocation().getX()
                                            - (double) (this.location().getBlockX())),
                            (double) (target.location().getBlockY())
                                    + (entity.getLocation().getY()
                                            - (double) (this.location().getBlockY())),
                            (double) (target.location().getBlockZ())
                                    + (entity.getLocation().getZ()
                                            - (double) (this.location().getBlockZ())));
    ...
    public BoundingBox getBoundingBox() {
        ...
        return new BoundingBox(
                ...
                loc.getBlockY(),
                loc.getBlockZ(),
                loc.getBlockX() + TelepadManager.horizontalSize,
                loc.getBlockY() + TelepadManager.verticalReach,
    ...
    public boolean inTelepad(Location location) {
        return this.location().getWorld() == location.getWorld()
                && this.getBoundingBox().contains(location.toVector());
    }
            ...
            for (int y = 0; y < TelepadManager.verticalReach; y++)
                for (int z = 0; z < TelepadManager.horizontalSize; z++)
                    if (!this.location()
                            .getWorld()
                            .getBlockAt(
                                    this.location().getBlockX() + x,
                                    this.location().getBlockY() + y,
                                    this.location().getBlockZ() + z)
                            .isEmpty()) return true;
        return false;
    }
}
