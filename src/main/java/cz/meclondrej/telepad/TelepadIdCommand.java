	...
	public boolean handle(CommandSender exec, Command cmd, String alias, String[] args) {
		...
		if (args.length < 2) {
			...
			player.sendMessage(Plugin
					.formatMessage("%s: %d %d %d".formatted(target.label(), target.location().getBlockX() + halfSize,
							target.location().getBlockY(), target.location().getBlockZ() + halfSize)));
			return true;
		}
		...
		for (Telepad telepad : TelepadManager.telepads)
			if (telepad.label().equals(args[1])) {
				exec.sendMessage(Plugin.formatMessage(
						"%s: %d %d %d".formatted(telepad.label(), telepad.location().getBlockX() + halfSize,
								telepad.location().getBlockY(), telepad.location().getBlockZ() + halfSize)));
				return true;
			}
